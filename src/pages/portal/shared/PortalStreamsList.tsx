...
interface PortalStreamsListProps {
  role: "admin" | "editor";
  baseUrl: string;
}

export default function PortalStreamsList({ role, baseUrl }: PortalStreamsListProps) {
  ...
  const { data: streams, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("live_streams").select("*").order("scheduled_at", { ascending: false });
  ...
  const toggleLive = async (id: string, isLive: boolean) => {
    ...
    const { error } = await supabase.from("live_streams").update({ is_live: !isLive }).eq("id", id);
    ...
    if (error) {
      ...
      if (!isLive && stream) {
        try {
          ...
          toast.success("🔴 Stream is now live! Subscribers notified.");
        } catch {
  ...
  return (
    <PortalLayout requiredRole={role}>
      <Helmet>
        <title>Live Streams | Portal</title>
      </Helmet>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Live Streams</h1>
        <Button asChild>
          <Link to={`${baseUrl}/streams/new`}>
      ...
      <div className="mb-6">
        <div className="relative max-w-sm">
          ...
          <Input placeholder="Search streams..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10" />
        </div>
      </div>
      ...
      {isLoading ? (
            ...
            <TableBody>
              {filteredStreams.map((stream) => (
                <TableRow key={stream.id}>
                  ...
                  <TableCell className="text-muted-foreground">{new Date(stream.scheduled_at).toLocaleString()}</TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-2">
                      <Button variant={stream.is_live ? "destructive" : "default"} size="sm" onClick={() => toggleLive(stream.id, stream.is_live || false)}>
                        {stream.is_live ? "End Stream" : "Go Live"}
                      </Button>
                      <Button variant="ghost" size="icon" asChild>
                        <Link to={`${baseUrl}/streams/${stream.id}`}>
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>Are you sure you want to delete this live stream? This action cannot be undone.</AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
        ...
        <div className="text-center py-12 border rounded-lg">
          ...
          <Button asChild>
            <Link to={`${baseUrl}/streams/new`}>Schedule your first stream</Link>
          </Button>
        </div>
      )}
    </PortalLayout>
